export default function CentralCore({ active = false }) {
  return (
    ...
    >
      ...
      <svg
        className="absolute h-[78%] w-[78%] animate-spin-mid text-red-500/30"
        viewBox="0 0 200 200"
      >
      ...
      <div
        className="absolute h-[42%] w-[42%] rounded-full border border-red-400/40"
        style={{
          boxShadow:
            "0 0 40px rgba(239, 68, 68, 0.35), inset 0 0 30px rgba(249, 115, 22, 0.15)",
        }}
      />

      <div
        className={`relative h-[22%] w-[22%] rounded-full bg-gradient-to-br from-red-400 via-orange-500 to-red-700 animate-pulse-core ${
          active
            ? "shadow-[0_0_60px_rgba(239,68,68,0.9)]"
            : "shadow-[0_0_40px_rgba(239,68,68,0.55)]"
        }`}
      >
        <div className="absolute inset-[18%] rounded-full bg-white/25 blur-sm" />
      </div>
    </div>
  );
}
